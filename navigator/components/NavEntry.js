...
export class NavEntry {
	...
	constructor(path, stat, nav_window_ref) {
		...
		let badge = this.dom_element.nav_item_badge = document.createElement("span");
		badge.classList.add("nav-item-badge");
		badge.style.display = "none";

		this.dom_element.appendChild(icon);
		this.dom_element.appendChild(badge);
		...
		if (nav_window_ref && nav_window_ref.item_display === "list") {
			...
			mode.innerText = stat ? stat["mode-str"] : "";
			owner.innerText = stat ? stat["owner"] : "";
			group.innerText = stat ? stat["group"] : "";
			size.innerText = stat ? format_bytes(stat["size"]) : "";
			modified.innerText = stat ? format_time(stat["mtime"]) : "";
			created.innerText = stat ? format_time(stat["ctime"]) : "";
